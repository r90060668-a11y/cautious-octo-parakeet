...
import com.kiro.arcade.ui.screens.HostInfo
...
object FirebaseRepository {
    ...
    suspend fun register(email: String, password: String, username: String): Result<Unit> {
        return try {
            ...
            val user = hashMapOf(
                "uid" to uid, "username" to username, "bio" to "",
                "avatarUrl" to "", "followersCount" to 0, "followingCount" to 0, "fcmToken" to ""
            )
    ...
    suspend fun followUser(targetUid: String): Result<Unit> {
        ...
        return try {
            ...
            if (theyFollowMe) createChatIfNeeded(uid, targetUid)
    ...
    private suspend fun createChatIfNeeded(uid1: String, uid2: String) {
        ...
        if (!exists) {
            chatRef.set(mapOf(
                ...
                "lastMessage" to "", "lastMessageTime" to System.currentTimeMillis()
            )).await()
        }
    }
    ...
    private suspend fun sendFollowNotification(targetUid: String) {
        ...
        db.collection("notifications").add(mapOf(
            "toUid" to targetUid, "fromUid" to (currentUid ?: ""),
            "fromUsername" to me.username, "type" to "follow",
    ...
    suspend fun createPost(caption: String, imageUrl: String): Result<Unit> {
        ...
        return try {
            ...
            postRef.set(hashMapOf(
                "id" to postRef.id, "authorUid" to uid,
                "authorUsername" to (user?.username ?: ""),
                "authorAvatarUrl" to (user?.avatarUrl ?: ""),
                "imageUrl" to imageUrl, "caption" to caption,
                "likesCount" to 0, "timestamp" to System.currentTimeMillis()
            )).await()
            Result.success(Unit)
        } catch (e: Exception) { Result.failure(e) }
    }
        ...
        return try {
            if (like) {
                db.collection("posts").document(postId)
                    .set(mapOf("likesCount" to FieldValue.increment(1)), SetOptions.merge()).await()
                db.collection("posts").document(postId)
            ...
            .addSnapshotListener { snap, error ->
                ...
                val chats = snap?.documents?.mapNotNull { doc ->
                    try {
                        ...
                        Chat(
                            id = doc.id, participants = participants,
    ...
    suspend fun sendMessage(chatId: String, text: String): Result<Unit> {
        ...
        return try {
            ...
            msgRef.set(hashMapOf(
                "id" to msgRef.id, "senderUid" to uid,
                "text" to text, "timestamp" to System.currentTimeMillis()
            )).await()
    ...
    suspend fun createHost(name: String, version: String, networkId: String): Result<Unit> {
        ...
        return try {
            ...
            ref.set(hashMapOf(
                "id" to ref.id, "name" to name, "version" to version,
                "networkId" to networkId, "ownerUid" to uid,
                "ownerUsername" to (user?.username ?: ""),
                "playerCount" to 0, "maxPlayers" to 25,
                "online" to true, "timestamp" to System.currentTimeMillis()
            )).await()
            Result.success(Unit)
        } catch (e: Exception) { Result.failure(e) }
    }

    suspend fun getHosts(): List<HostInfo> {
        return try {
            db.collection("hosts")
                .orderBy("timestamp", Query.Direction.DESCENDING)
                .get().await()
